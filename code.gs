...
function onOpen(e) {
 ...
 // --- Daily Usage Tracking menu ---
 ui.createMenu('📊 Daily Usage Tracking')
...
function backfillDailyStudioUsage() {
  ...
  sheets.forEach(sheet => {
    // Skip usage sheets themselves
    if (sheet.getName().indexOf('Usage') >= 0) return;
  ...
  SpreadsheetApp.getUi().alert('Backfill complete! Processed ' + totalDays + ' day(s) from ' + startOfYear.toLocaleDateString() + ' to ' + today.toLocaleDateString());
}
